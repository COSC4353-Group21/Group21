...
describe("The login module", () => {
  ...
  test("allows valid username and password for registration", async () => {
    return expect(
      create_user(valid_user.username, valid_user.password)
  ...
  test("allows valid users to log in", async () => {
    ...
    return expect(
      generate_token(valid_user.username, valid_user.password)
    ).resolves.not.toBeUndefined();
  });
  test("forbid nonexistent users to log in", async () => {
    return expect(
      generate_token(invalid_user.username, invalid_user.password)
    ).rejects.toThrow();
  });
  test("returns an error for incorrect password", async () => {
    await create_user(valid_user.username, valid_user.password);
    return expect(
      generate_token(bad_password.username, bad_password.password)
    ).rejects.toThrow();
  });
  test("generates valid tokens on successful login", async () => {
    await create_user(valid_user.username, valid_user.password);
    return expect(
      generate_token(valid_user.username, valid_user.password).then((token) => {
  ...
  test("does not validate forged tokens", async () => {
    await create_user(valid_user.username, valid_user.password);
    return expect(
      validate_token(valid_user.username, fake_token)
    ).resolves.toBe(false);
  });
  test("throws error if logout is attempted on an invalid token", async () => {
    await create_user(valid_user.username, valid_user.password);
    return expect(
      invalidate_token(valid_user.username, fake_token)
    ).rejects.toThrow();
  });
  test("successfully invalidates a valid token on log out", async () => {
    ...
    return expect(
      invalidate_token(valid_user.username, token)
    ).rejects.toThrow();
  });
