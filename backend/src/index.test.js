...
describe("The Express app", () => {
  ...
  test("should allow users to view their profile", async () => {
    ...
    await clientWithAuth(token)
      ...
      .then((response) => {
        const { full_name, address1, address2, city, state, zipcode } =
          response.data;
  ...
  test("should allow users to edit their profile", async () => {
    ...
    await clientWithAuth(token)
      .get("/api/profile/" + username)
      .then((response) => {
        const { fullname, address1, address2, city, state, zipcode } =
          response.data;
  ...
  test("should prevent users from accessing profile with bogus tokens", async () => {
    await profile_shim();
    return expect(
      clientWithAuth("badtoken").get("/api/profile/richard")
    ).rejects.toThrow();
  });
  ...
  test("should prevent users from editing profile with bogus tokens", async () => {
    await login_shim();
    return expect(
      clientWithAuth("badtoken").post("/api/profile", profile)
    ).rejects.toThrow();
  });
